...
do
  ...
  # make a dir
  mkdir "_batch_surf_${surf}_${phi}"

  # copy the compute data
  cp "./compute_data.py" "_batch_surf_${surf}_${phi}/compute_data.py"

  # write the run file
  RUN="_batch_surf_${surf}_${phi}/run.sh"
  if [ ! -f "${RUN}" ]; then
  ...
  SUB="_batch_surf_${surf}_${phi}/submit.sub"
  if [ -f "${SUB}" ]; then
    rm "${SUB}"
  fi
  printf '%s\n' "#!/bin/bash" >> ${SUB}
  printf '%s\n' "#SBATCH -J bdry_${surf}_${phi} # Job name" >> ${SUB}
  ...
  printf '%s\n' "mpiexec -n $[$NODES*$CORES] python3 compute_data.py ${surf} ${phi}" >> ${SUB}
  
  ## submit
  cd "./_batch_surf_${surf}_${phi}"
  "./run.sh"
  cd ..
