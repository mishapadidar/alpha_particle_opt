...
import numpy as np
import time 
from bfield_axisymmetry import AxisymmetricField

"""
Track a blob of probability mass through an Axisymmetric B field
WITHOUT drift computations. 

An axisymmetric field takes the form
  B = grad(psi) x grad(phi) + G* grad(phi)
where
  psi = (R-R0)^2 + Z^2
and (R,phi,Z) are standard cylindrical coordinates and G is constant.

Because we are not injecting a current into the plasma volume, 
alpha particles will not be confined. However, if we turn off
the drift terms in the guiding center equations we expect 
perfect confinement even without a current.
...
zmin = 0.0
# plot the field
bs.to_vtk('magnetic_field_ex3',nphi=32,rmin=rmin,rmax=rmax,zmin=zmin,zmax=zmax)
