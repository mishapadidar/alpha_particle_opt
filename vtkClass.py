...
# MISHA IS NOW TAMPERING WITH THE CODE SO HE CAN SEND THE .vtu FILES TO A DIFFERENT DIRECTORY.
    # The .pvd file will now know where to find the .vtu files. previously the .pvd file would search for the .vtu files as
    # '/directory/file.vtu' instead of 'file.vtu'. This code block removes the '/directory/' part
        if '/' in fileName:
            for character in range(len(fileName)-1,0,-1):
                if fileName[character]=='/':
                    fileName =fileName[character+1:]
                    break
# MISHA IS NOW DONE TAMPERING WITH CODE. TO CHANGE CODE TO ORIGINAL, REMOVE THIS SECTION.
